...
__export(main_exports, {
  default: () => DailyWeeklyNotesPlugin
});
...
var DailyWeeklyNotesPlugin = class extends import_obsidian.Plugin {
  async onload() {
  ...
  async createDailyNote() {
    try {
      ...
      new import_obsidian.Notice("Failed to create daily note. Check console for details.");
    }
  }
  async createWeeklyNote() {
    try {
      ...
      new import_obsidian.Notice("Failed to create weekly note. Check console for details.");
    }
  }
  generateDailyNoteContent(date) {
    try {
...
`;
  ...
  generateWeeklyNoteContent(date) {
    try {
...
`;
  ...
  async createOrUpdateNote(filePath, content, noteType) {
    try {
      ...
      if (file instanceof import_obsidian.TFile) {
        ...
        new import_obsidian.Notice(`${noteType === "daily" ? "Daily" : "Weekly"} note template added to existing file`);
      } else {
        await this.app.vault.create(filePath, content);
        new import_obsidian.Notice(`${noteType === "daily" ? "Daily" : "Weekly"} note created`);
      }
      ...
      new import_obsidian.Notice(`Failed to create ${noteType} note. The file may be open in another program or the format may be invalid.`);
      throw error;
    }
  }
};
var DailyWeeklyNotesSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    dailyPreview.setText(`Preview: ${(0, import_moment.default)().format(this.plugin.settings.dailyNoteFormat)}`);
    new import_obsidian.Setting(containerEl).setName("Daily note filename format").setDesc("Format for daily note filenames and links (e.g., YYYY-MM-DD)").addText((text) => text.setPlaceholder("YYYY-MM-DD").setValue(this.plugin.settings.dailyNoteFormat).onChange(async (value) => {
      this.plugin.settings.dailyNoteFormat = value;
      await this.plugin.saveSettings();
      try {
        dailyPreview.setText(`Preview: ${(0, import_moment.default)().format(value)}`);
      } catch (e) {
    ...
    weeklyPreview.setText(`Preview: ${(0, import_moment.default)().format(this.plugin.settings.weeklyNoteFormat)}`);
    new import_obsidian.Setting(containerEl).setName("Weekly note filename format").setDesc("Format for weekly note filenames and links (use W for ISO week number, GGGG for ISO week year, [text] for literal text)").addText((text) => text.setPlaceholder("GGGG - [Week] W").setValue(this.plugin.settings.weeklyNoteFormat).onChange(async (value) => {
      this.plugin.settings.weeklyNoteFormat = value;
      await this.plugin.saveSettings();
      try {
        weeklyPreview.setText(`Preview: ${(0, import_moment.default)().format(value)}`);
      } catch (e) {
    ...
    const weekStart = (0, import_moment.default)().isoWeekday(1);
    const weekEnd = weekStart.clone().add(6, "days");
    ...
    new import_obsidian.Setting(containerEl).setName("Weekly note date range format").setDesc("Format for the date range shown in weekly notes (e.g., MMMM Do)").addText((text) => text.setPlaceholder("MMMM Do").setValue(this.plugin.settings.weeklyDateRangeFormat).onChange(async (value) => {
      this.plugin.settings.weeklyDateRangeFormat = value;
      await this.plugin.saveSettings();
      try {
        const weekStart2 = (0, import_moment.default)().isoWeekday(1);
        const weekEnd2 = weekStart2.clone().add(6, "days");
        rangePreview.setText(`Preview: ${weekStart2.format(value)} - ${weekEnd2.format(value)}`);
      } catch (e) {
